    ...
    function async_training_loop(callback) {
      (function loop(i) {
        ...
        for(var k = 0; k < Math.floor(X_scaled.shape[0]/timestamp)*timestamp; k+=timestamp){
          ...
          total_Profit += parseFloat(cost(batch_y,f(batch_x,batch_states)['forward']).toString().slice(7));
          batch_states = [feed.states_1,feed.states_2]
        }
        total_Profit /= Math.floor(X_scaled.shape[0]/timestamp);
        arr_Profit.push(total_Profit)
        ...
        $('#log').append('Epoch: '+(i+1)+', avg Profit: '+total_Profit+'<br>');
        ...
        option = {
          ...
          xAxis: {
            type: 'category',
            data: arange(0,arr_Profit.length,1)
          },
          yAxis: {
            type: 'value'
          },
        ...
        var chart_line = echarts.init(document.getElementById('div_Profit'));
