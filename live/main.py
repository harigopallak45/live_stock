...
def init():
    now = dt.datetime.now()
    ax.set_xlim(now, now + dt.timedelta(seconds=20))
...
def update(frame):
    ...
    # Generate the current timestamp incrementally
    if timestamps:
        timestamp = timestamps[-1] + dt.timedelta(seconds=1)
    else:
    ...
    # Adjust x-axis limits to show the most recent 20 seconds
    if len(timestamps) > 20:
        ax.set_xlim(timestamps[-20], timestamps[-1] + dt.timedelta(seconds=1))
    else:
        ax.set_xlim(timestamps[0], timestamps[-1] + dt.timedelta(seconds=1))
...
# Show plot with navigation toolbar
plt.show()
